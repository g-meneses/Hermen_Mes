...
/**
 * Verificar si un cÃ³digo ya existe en la base de datos (API async)
 */
async function verificarCodigoDuplicado(codigo, excludeId = null) {
    ...
    try {
        ...
        if (d.existe) {
            actualizarBannerCodigo('error', `âŒ ERROR: CÃ³digo duplicado. Ya pertenece a: "${d.producto_existente}". Ajuste el sufijo.`);
            codigoValidado = false;
            actualizarEstadoBotonGuardar();
            return false;
        } else {
...
async function cargarDatos() {
    ...
    // Si hay una vista activa, refrescarla para que se reflejen los cambios (ej: eliminaciÃ³n)
    if (subcategoriaSeleccionada) {
        ...
        // En teorÃ­a, si hay categorÃ­a pero no subcategorÃ­a seleccionada, estamos viendo subcategorÃ­as o productos
...
function abrirModalNuevoItem() {
    lastLoadedCategoryId = null; // Resetear cachÃ© de subcategorÃ­as
...
function poblarSelects() {
    // Poblar categorÃ­as (usar ID con sufijo _modal)
    const catSelect = document.getElementById('itemCategoria_modal');
    if (catSelect) {
        const currentCat = catSelect.value;
        catSelect.innerHTML = '<option value="">Seleccione categorÃ­a...</option>' +
            categorias.map(c => `<option value="${c.id_categoria}">${c.nombre}</option>`).join('');
        if (currentCat) catSelect.value = currentCat;
    }

    // Poblar subcategorÃ­as (usar ID con sufijo _modal)
    const subcatSelect = document.getElementById('itemSubcategoria_modal');
    if (subcatSelect) {
        subcatSelect.innerHTML = '<option value="0">Sin subcategorÃ­a</option>';
    }
...
async function eliminarItem(id, codigo, nombre) {
    // Confirmar eliminaciÃ³n con SweetAlert2
    const result = await Swal.fire({
        icon: 'warning',
        title: 'Â¿Eliminar producto?',
        html: `<strong>CÃ³digo:</strong> ${codigo}<br>` +
...
async function actualizarNumeroSalida() {
    ...
    try {
        ...
        const url = `${BASE_URL_API}/salidas_emp.php?action=siguiente_numero&tipo=${tipo}&destino=${destino}`;
...
function ejecutarFallbackLocalSalida(tipo, destino, motivo) {
    const prefijosBase = { 'PRODUCCION': 'OUT-EMP-P', 'VENTA': 'OUT-EMP-V', 'MUESTRAS': 'OUT-EMP-M', 'AJUSTE': 'OUT-EMP-A', 'DEVOLUCION': 'OUT-EMP-R' };
    const prefijosDestino = { 'TEJIDO': 'SAL-TEJ', 'COSTURA': 'SAL-COS', 'TENIDO': 'SAL-TEN' };

    let prefijo = prefijosBase[tipo] || 'OUT-EMP-X';
    ...
    if (docInput) {
        ...
        docInput.disabled = false;
        console.warn('Fallback EMP:', numero);
    }
}
...
console.log('âœ… MÃ³dulo Material de Empaque v1.9 cargado');
