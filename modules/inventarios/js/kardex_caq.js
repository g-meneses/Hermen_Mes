// ========== KARDEX FÍSICO Y VALORADO DE COLORANTES Y AUX. QUÍMICOS ==========
...
async function buscarKardex() {
    ...
    try {
        ...
        const response = await fetch(`${baseUrl}/api/kardex_caq.php?${params}`);
...
function exportarKardex() {
    ...
    // window.open(`${baseUrl}/api/kardex_caq.php?${params}`, '_blank');
}
...
async function recalcularKardex(idInventario) {
    ...
    try {
        const response = await fetch(`${baseUrl}/api/kardex_caq.php?action=recalcular&id_inventario=${idInventario}`);
        const data = await response.json();
