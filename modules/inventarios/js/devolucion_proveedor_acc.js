// ========== MODAL DEVOLUCIÓN A PROVEEDOR - ACCESORIOS (ACC) ==========
...
async function abrirModalDevolucion() {
    ...
    // Generar número
    document.getElementById('devolucionDocumento').value = generarNumeroDoc('SACC-DV');
...
async function cargarIngresosDisponibles() {
    try {
        const r = await fetch(`${baseUrl}/api/salidas_acc.php?action=ingresos_devolucion&limit=20`);
...
function renderIngresosDisponibles(ingresos) {
    ...
    container.innerHTML = ingresos.map(ing => {
        ...
        // Algunas veces 'total' viene como string, asegurar float
        const total = parseFloat(ing.total);
        // Verificar campo con_factura, si no existe asumir false o verificar lógica
        const conFactura = ing.con_factura ? (parseInt(ing.con_factura) === 1) : false;

        // NOTA: Si salidas_acc.php no devuelve con_factura en 'ingresos_devolucion', 
        // podría faltar en la query. Revisamos api/salidas_acc.php y NO tiene con_factura en SELECT.
        // Asumo que lo agregaremos o manejar sin factura por ahora.
        // En api/salidas_acc.php linea 152 no está con_factura. 
        // Sin embargo, para no romper, mostramos badge genérico si falta.
        ...
        return `
            <div class="ingreso-card" onclick="seleccionarIngreso(${ing.id_documento})" 
                ...
                <div class="ingreso-total">
                    Total: <strong>Bs. ${formatNum(total, 2)}</strong>
                </div>
            </div>`;
    }).join('');
}
...
async function seleccionarIngreso(idDocumento) {
    try {
        ...
        // Cargar detalle
        const r = await fetch(`${baseUrl}/api/salidas_acc.php?action=detalle_ingreso&id_ingreso=${idDocumento}`);
        const d = await r.json();

        if (d.success) {
            ingresoSeleccionado = d.documento || { id_documento: idDocumento }; // Fallback si API no devuelve documento completo
            // La API salidas_acc.php devuelve 'detalle' pero no 'documento' en 'detalle_ingreso'.
            // Debemos arreglar la API o simular el objeto documento.
            // En salidas_acc.php linea 208 solo devuelve 'detalle'.
            // Necesitamos la info del ingreso (id_proveedor) para guardar.
            // Lo tomamos del array 'todosLosIngresosDisponibles'.

            const ingInfo = todosLosIngresosDisponibles.find(i => i.id_documento == idDocumento);
            if (ingInfo) {
                ingresoSeleccionado = { ...ingInfo, ...ingresoSeleccionado };
            }
            ...
            lineasDevolucion = d.detalle.map(linea => ({
                ...
                unidad: linea.unidad,
                cantidad_original: parseFloat(linea.cantidad_ingresada || linea.cantidad), // API devuelve cantidad_ingresada
                cantidad_devuelta: 0, // TODO: La API debería devolver cuánto ya se devolvió
                cantidad_disponible: parseFloat(linea.cantidad_ingresada || linea.cantidad), // Asumimos todo disponible por ahora
                cantidad_a_devolver: 0,
                costo_unitario: parseFloat(linea.costo_unitario),
                costo_con_iva: parseFloat(linea.costo_unitario), // Simplificación
                tenia_iva: false // Simplificación
            }));
...
function renderLineasDevolucion() {
    ...
    const conFactura = false; // ingresoSeleccionado && parseInt(ingresoSeleccionado.con_factura) === 1;
...
async function guardarDevolucion() {
    ...
    const conFactura = false; // parseInt(ingresoSeleccionado.con_factura) === 1;

    const data = {
        action: 'crear',
        tipo_salida: 'DEVOLUCION_PROV', // Ajustado a lo que espera salidas_acc.php (ver switch tiposMovimiento)
        ...
        lineas: lineasConCantidad.map(l => ({
            ...
            // Para salidas_acc.php API estándar:
            subtotal: (l.cantidad_a_devolver * l.costo_unitario)
        }))
    };
    ...
    try {
        const r = await fetch(`${baseUrl}/api/salidas_acc.php`, {
            method: 'POST',
...
`;
if (!document.getElementById('estilosDevolucion')) {
    const divEstilos = document.createElement('div');
    divEstilos.id = 'estilosDevolucion';
    divEstilos.innerHTML = estilosDevolucion;
    document.body.appendChild(divEstilos);
}

console.log('✅ Módulo de Devoluciones ACC cargado');
