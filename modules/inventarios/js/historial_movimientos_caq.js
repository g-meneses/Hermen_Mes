// ========== MODAL HISTORIAL DE MOVIMIENTOS - CAQ ==========
...
async function buscarHistorial() {
    ...
    try {
        ...
        const [rIngresos, rSalidas] = await Promise.all([
            fetch(`${baseUrl}/api/ingresos_caq.php?action=list&${params}`),
            fetch(`${baseUrl}/api/salidas_caq.php?action=list&${params}`)
        ]);
...
async function verDetalleDocumento(idDocumento, tipo) {
    try {
        let url = '';
        if (tipo === 'INGRESO') {
            url = `${baseUrl}/api/ingresos_caq.php?action=get&id=${idDocumento}`;
        } else {
            url = `${baseUrl}/api/salidas_caq.php?action=get&id=${idDocumento}`;
        }
...
async function anularDocumento(idDocumento, tipo, motivo) {
    try {
        let url = '';
        if (tipo === 'INGRESO') {
            url = `${baseUrl}/api/ingresos_caq.php`;
        } else {
            url = `${baseUrl}/api/salidas_caq.php`;
        }
