...
document.addEventListener('DOMContentLoaded', function () {
    console.log('🚀 Inicializando módulo de Material de Empaque...');
...
async function obtenerSiguienteNumero(tipo = null) {
    try {
        ...
        const url = `${BASE_URL_API}/ingresos_emp.php?action=siguiente_numero&tipo=${tipo}`;
        const response = await fetch(url);
        const data = await response.json();
...
async function cargarCategoriasIngreso() {
    try {
        const url = `${BASE_URL_API}/centro_inventarios.php?action=categorias&tipo_id=3`;
