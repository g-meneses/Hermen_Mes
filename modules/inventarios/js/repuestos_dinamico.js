...
document.addEventListener('DOMContentLoaded', function () {
    console.log('ðŸš€ Inicializando mÃ³dulo de Repuestos y MÃ¡quinas...');
...
async function obtenerSiguienteNumero(tipo = null) {
    ...
    try {
        ...
        const url = ${BASE_URL_API}/ingresos_rep.php?action=siguiente_numero&tipo=;
...
function ejecutarFallbackLocalIngreso(tipo, motivo) {
    ...
    const numero = 'IN-REP-' + movCode + '-' + fechaStr + '-' + rand;
    numerosDocumentoCache[tipo] = numero;
    docInput.value = numero;
    docInput.disabled = false;
    console.warn('Fallback IN REP:', numero);
}
...
async function cargarCategoriasIngreso() {
    try {
        const url = `${BASE_URL_API}/centro_inventarios.php?action=categorias&tipo_id=${window.TIPO_INVENTARIO_ID || 7}`;
