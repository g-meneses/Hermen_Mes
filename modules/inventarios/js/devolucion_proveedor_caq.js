// ========== MODAL DEVOLUCIÓN A PROVEEDOR - CAQ ==========
...
async function abrirModalDevolucion() {
    ...
    // Generar número
    document.getElementById('devolucionDocumento').value = generarNumeroDoc('SCAQ-DV');
...
async function cargarIngresosDisponibles() {
    try {
        const r = await fetch(`${baseUrl}/api/salidas_caq.php?action=ingresos_devolucion&limit=20`);
...
async function seleccionarIngreso(idDocumento) {
    try {
        ...
        // Cargar detalle
        const r = await fetch(`${baseUrl}/api/salidas_caq.php?action=detalle_ingreso&id=${idDocumento}`);
        const d = await r.json();
