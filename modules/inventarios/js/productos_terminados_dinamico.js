...
document.addEventListener('DOMContentLoaded', function () {
    console.log('🚀 Inicializando módulo de Productos Terminados...');
...
async function obtenerSiguienteNumero(tipo = null) {
    try {
        ...
        const url = `${BASE_URL_API}/ingresos_pt.php?action=siguiente_numero&tipo=${tipo}`;
        const response = await fetch(url);
        const data = await response.json();
...
async function cargarCategoriasIngreso() {
    try {
        const url = `${BASE_URL_API}/centro_inventarios.php?action=categorias&tipo_id=${window.TIPO_INVENTARIO_ID || 6}`;
