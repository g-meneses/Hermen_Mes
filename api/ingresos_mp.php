 ...
 * Versión: 2.1 - Compatible con tipos de ingreso
 * Fecha: 2026-01-08
 */
            ...
            switch ($action) {
                case 'list':
                    ...
                    $sql = "SELECT 
                                ...
                                d.numero_documento,
                                d.fecha_documento,
                                d.tipo_documento COLLATE utf8mb4_unicode_ci AS tipo_documento,
                                d.tipo_ingreso,
                                d.id_tipo_ingreso,
                            ...
                            WHERE d.tipo_documento COLLATE utf8mb4_unicode_ci = 'INGRESO' 
                    ...
                    if ($estado !== 'todos') {
                        $sql .= " AND d.estado COLLATE utf8mb4_unicode_ci = ?";
                        $params[] = $estado;
                    }
                ...
                case 'kardex':
                    ...
                    $stmt = $db->prepare("
                        SELECT 
                            m.*,
                            CASE 
                                WHEN m.tipo_movimiento COLLATE utf8mb4_unicode_ci LIKE 'ENTRADA%' THEN 'ENTRADA'
                                WHEN m.tipo_movimiento COLLATE utf8mb4_unicode_ci LIKE 'SALIDA%' THEN 'SALIDA'
                                ELSE 'INTERNO'
                            END AS direccion
                        FROM movimientos_inventario m
                        WHERE m.id_inventario = ?
                        AND m.estado COLLATE utf8mb4_unicode_ci = 'ACTIVO'
            ...
            switch ($action) {
                case 'crear':
                    ...
                    try {
                        ...
                        $totalNeto = 0;
                        $iva = 0;
                        $conFactura = isset($data['con_factura']) && $data['con_factura'];
                        ...
                        $stmt = $db->prepare("
                            INSERT INTO documentos_inventario (
                                ...
                                id_area_produccion, motivo_ingreso, referencia_externa,
                ...
                case 'anular':
                    ...
                    try {
                        // Verificar documento
                        $stmt = $db->prepare("SELECT * FROM documentos_inventario WHERE id_documento = ? AND estado COLLATE utf8mb4_unicode_ci = 'CONFIRMADO'");
                        $stmt->execute([$id]);
                        ...
                        $stmtMov = $db->prepare("SELECT * FROM movimientos_inventario WHERE documento_id = ? AND estado COLLATE utf8mb4_unicode_ci = 'ACTIVO'");
                        $stmtMov->execute([$id]);
